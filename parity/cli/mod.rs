...
usage! {
	...
	}
	{
		...
		["Operating Options"]
			...
			"--public-node 'Start Parity as a public web server. Account storage and transaction signing will be delegated to the UI.'",

			FLAG flag_no_download: bool = false, or |c: &Config| otry!(c.parity).no_download.clone(),
			"--no-download 'Normally new releases will be downloaded ready for updating. This disables it. Not recommended.'",

			FLAG flag_no_consensus: bool = false, or |c: &Config| otry!(c.parity).no_consensus.clone(),
			"--no-consensus 'Force the binary to run even if there are known issues regarding consensus. Not recommended.'",

			FLAG flag_light: bool = false, or |c: &Config| otry!(c.parity).light,
			"--light 'Experimental: run in light client mode. Light clients synchronize a bare minimum of data and fetch necessary data on-demand from the network. Much lower in storage, potentially higher in bandwidth. Has no effect with subcommands.'",

			ARG arg_mode: String = "last", or |c: &Config| otry!(c.parity).mode.clone(),
			"--mode=[MODE] 'Set the operating mode. MODE can be one of:{n}
			...
			"--mode-timeout=[SECS] 'Specify the number of seconds before inactivity timeout occurs when mode is dark or passive'",

			ARG arg_mode_alarm: u64 = 3600u64, or |c: &Config| otry!(c.parity).mode_alarm.clone(),
			"--mode-alarm=[SECS] 'Specify the number of seconds before auto sleep reawake timeout occurs when mode is passive'",

			ARG arg_auto_update: String = "critical", or |c: &Config| otry!(c.parity).auto_update.clone(),
			"--auto-update=[SET] 'Set a releases set to automatically update and install.{n}
			...
			"--release-track=[TRACK] 'Set which release track we should use for updates.{n}
			...
			"--chain=[CHAIN] 'Specify the blockchain type. CHAIN may be either a JSON chain specification file or olympic, frontier, homestead, mainnet, morden, ropsten, classic, expanse, testnet, kovan or dev.'",
		...
		["Convenience options"]
			...
			"--unsafe-expose 'All servers will listen on external interfaces and will be remotely accessible. It's equivalent with setting the following: --{{ws,jsonrpc,ui,ipfs,secret_store,stratum}}-interface=all --*-hosts=all{n}This option is UNSAFE and should be used with great care!'",
			...
			"--ports-shift=[SHIFT] 'Add SHIFT to all port numbers Parity is listening on. Includes network port and all servers (RPC, WebSockets, UI, IPFS, SecretStore).'",
		...
		["Account options"]
			...
			"--fast-unlock 'Use drasticly faster unlocking mode. This setting causes raw secrets to be stored unprotected in memory, so use with care.'",

			ARG arg_keys_iterations: u32 = 10240u32, or |c: &Config| otry!(c.account).keys_iterations.clone(),
			"--keys-iterations=[NUM] 'Specify the number of iterations to use when deriving key from the password (bigger is more secure)'",
		...
		["UI options"]
			...
			"--force-ui 'Enable Trusted UI WebSocket endpoint, even when --unlock is in use.'",
			...
			"--ui-no-validation 'Disable Origin and Host headers validation for Trusted UI. WARNING: INSECURE. Used only for development.'",

			ARG arg_ui_interface: String = "local", or |c: &Config| otry!(c.ui).interface.clone(),
			"--ui-interface=[IP] 'Specify the hostname portion of the Trusted UI server, IP should be an interface's IP address, or local.'",

			ARG arg_ui_hosts: String = "none", or |c: &Config| otry!(c.ui).hosts.as_ref().map(|vec| vec.join(",")),
			"--ui-hosts=[HOSTS] 'List of allowed Host header values. This option will validate the Host header sent by the browser, it is additional security against some attack vectors. Special options: \"all\", \"none\",.'",

			ARG arg_ui_path: String = "$BASE/signer", or |c: &Config| otry!(c.ui).path.clone(),
			"--ui-path=[PATH] 'Specify directory where Trusted UIs tokens should be stored.'",
		...
		["Networking options"]
			...
			"--no-ancient-blocks 'Disable downloading old blocks after snapshot restoration or warp sync.'",

			FLAG flag_no_serve_light: bool = false, or |c: &Config| otry!(c.network).no_serve_light.clone(),
			"--no-serve-light 'Disable serving of light peers.'",
			...
			"--nat=[METHOD] 'Specify method to use for determining public address. Must be one of: any, none, upnp, extip:<IP>.'",

			ARG arg_allow_ips: String = "all", or |c: &Config| otry!(c.network).allow_ips.clone(),
			"--allow-ips=[FILTER] 'Filter outbound connections. Must be one of: private - connect to private network IP addresses only; public - connect to public network IP addresses only; all - connect to any IP address.'",

			ARG arg_max_pending_peers: u16 = 64u16, or |c: &Config| otry!(c.network).max_pending_peers.clone(),
			"--max-pending-peers=[NUM] 'Allow up to NUM pending connections.'",
		...
		["API and Console options: RPC"]
			...
			"--jsonrpc-interface=[IP] 'Specify the hostname portion of the JSONRPC API server, IP should be an interface's IP address, or all (all interfaces) or local.'",
			...
			"--jsonrpc-apis=[APIS] 'Specify the APIs available through the JSONRPC interface. APIS is a comma-delimited list of API name. Possible name are all, safe, web3, eth, net, personal, parity, parity_set, traces, rpc, parity_accounts. You can also disable a specific API by putting '-' in the front: all,-personal.'",

			ARG arg_jsonrpc_hosts: String = "none", or |c: &Config| otry!(c.rpc).hosts.as_ref().map(|vec| vec.join(",")),
			"--jsonrpc-hosts=[HOSTS] 'List of allowed Host header values. This option will validate the Host header sent by the browser, it is additional security against some attack vectors. Special options: \"all\", \"none\",.'",

			ARG arg_jsonrpc_threads: usize = 0usize, or |c: &Config| otry!(c.rpc).processing_threads,
			"--jsonrpc-threads=[THREADS] 'Turn on additional processing threads in all RPC servers. Setting this to non-zero value allows parallel cpu-heavy queries execution.'",
			...
			"--jsonrpc-server-threads=[NUM] 'Enables experimental faster implementation of JSON-RPC server. Requires Dapps server to be disabled using --no-dapps.'",

		["API and Console options: WS"]
			FLAG flag_no_ws: bool = false, or |c: &Config| otry!(c.websockets).disable.clone(),
			"--no-ws 'Disable the WebSockets server.'",
			...
			"--ws-interface=[IP] 'Specify the hostname portion of the WebSockets server, IP should be an interface's IP address, or all (all interfaces) or local.'",
			...
			"--ws-apis=[APIS] 'Specify the APIs available through the WebSockets interface. APIS is a comma-delimited list of API name. Possible name are web3, eth, pubsub, net, personal, parity, parity_set, traces, rpc, parity_accounts..'",

			ARG arg_ws_origins: String = "chrome-extension://*", or |c: &Config| otry!(c.websockets).origins.as_ref().map(|vec| vec.join(",")),
			"--ws-origins=[URL] 'Specify Origin header values allowed to connect. Special options: \"all\", \"none\".'",

			ARG arg_ws_hosts: String = "none", or |c: &Config| otry!(c.websockets).hosts.as_ref().map(|vec| vec.join(",")),
			"--ws-hosts=[HOSTS] 'List of allowed Host header values. This option will validate the Host header sent by the browser, it is additional security against some attack vectors. Special options: \"all\", \"none\",.'",
		...
		["API and Console options: IPC"]
			...
			"--ipc-apis=[APIS] 'Specify custom API set available via JSON-RPC over IPC.'",
		...
		["API and Console options: IPFS"]
			...
			"--ipfs-api 'Enable IPFS-compatible HTTP API.'",
			...
			"--ipfs-api-interface=[IP] 'Specify the hostname portion of the IPFS API server, IP should be an interface's IP address or local.'",

			ARG arg_ipfs_api_hosts: String = "none", or |c: &Config| otry!(c.ipfs).hosts.as_ref().map(|vec| vec.join(",")),
			"--ipfs-api-hosts=[HOSTS] 'List of allowed Host header values. This option will validate the Host header sent by the browser, it is additional security against some attack vectors. Special options: \"all\", \"none\".'",
		...
		["Secret store options"]
			...
			"--no-secretstore 'Disable Secret Store functionality.'",

			ARG arg_secretstore_nodes: String = "", or |c: &Config| otry!(c.secretstore).nodes.as_ref().map(|vec| vec.join(",")),
			"--secretstore-nodes=[NODES] 'Comma-separated list of other secret store cluster nodes in form NODE_PUBLIC_KEY_IN_HEX@NODE_IP_ADDR:NODE_PORT. (required, default: {arg_secretstore_nodes}).'",

			ARG arg_secretstore_interface: String = "local", or |c: &Config| otry!(c.secretstore).interface.clone(),
			"--secretstore-interface=[IP] 'Specify the hostname portion for listening to Secret Store Key Server internal requests, IP should be an interface's IP address, or local.'",

			ARG arg_secretstore_port: u16 = 8083u16, or |c: &Config| otry!(c.secretstore).port.clone(),
			"--secretstore-port=[PORT] 'Specify the port portion for listening to Secret Store Key Server internal requests.'",

			ARG arg_secretstore_http_interface: String = "local", or |c: &Config| otry!(c.secretstore).http_interface.clone(),
			"--secretstore-http-interface=[IP] 'Specify the hostname portion for listening to Secret Store Key Server HTTP requests, IP should be an interface's IP address, or local.'",

			ARG arg_secretstore_http_port: u16 = 8082u16, or |c: &Config| otry!(c.secretstore).http_port.clone(),
			"--secretstore-http-port=[PORT] 'Specify the port portion for listening to Secret Store Key Server HTTP requests.'",
			...
			"--secretstore-secret=[SECRET] 'Hex-encoded secret key of this node. (required, default: {arg_secretstore_secret:?}).'",
		...
		["Sealing/Mining options"]
			...
			"--reseal-on-uncle 'Force the node to author new blocks when a new uncle block is imported.'",
			...
			"--reseal-on-txs=[SET] 'Specify which transactions should force the node to reseal a block. SET is one of: none - never reseal on new transactions; own - reseal only on a new local transaction; ext - reseal only on a new external transaction; all - reseal on all new transactions.'",
			...
			"--usd-per-tx=[USD] 'Amount of USD to be paid for a basic transaction. The minimum gas price is set accordingly.'",

			ARG arg_usd_per_eth: String = "auto", or |c: &Config| otry!(c.mining).usd_per_eth.clone(),
			"--usd-per-eth=[SOURCE] 'USD value of a single ETH. SOURCE may be either an amount in USD, a web service or 'auto' to use each web service in turn and fallback on the last known good value.'",

			ARG arg_price_update_period: String = "hourly", or |c: &Config| otry!(c.mining).price_update_period.clone(),
			"--price-update-period=[T] 'T will be allowed to pass between each gas price update. T may be daily, hourly, a number of seconds, or a time string of the form \"2 days\", \"30 minutes\" etc..'",
			...
			"--gas-cap=[GAS] 'A cap on how large we will raise the gas limit per block due to transaction volume.'",

			ARG arg_tx_queue_mem_limit: u32 = 2u32, or |c: &Config| otry!(c.mining).tx_queue_mem_limit.clone(),
			"--tx-queue-mem-limit=[MB] 'Maximum amount of memory that can be used by the transaction queue. Setting this parameter to 0 disables limiting.'",

			ARG arg_tx_queue_size: usize = 8192usize, or |c: &Config| otry!(c.mining).tx_queue_size.clone(),
			"--tx-queue-size=[LIMIT] 'Maximum amount of transactions in the queue (waiting to be included in next block).'",

			ARG arg_tx_queue_gas: String = "off", or |c: &Config| otry!(c.mining).tx_queue_gas.clone(),
			"--tx-queue-gas=[LIMIT] 'Maximum amount of total gas for external transactions in the queue. LIMIT can be either an amount of gas or 'auto' or 'off'. 'auto' sets the limit to be 20x the current block gas limit..'",

			ARG arg_tx_queue_strategy: String = "gas_price", or |c: &Config| otry!(c.mining).tx_queue_strategy.clone(),
			"--tx-queue-strategy=[S] 'Prioritization strategy used to order transactions in the queue. S may be: gas - Prioritize txs with low gas limit; gas_price - Prioritize txs with high gas price; gas_factor - Prioritize txs using gas price and gas limit ratio.'",

			ARG arg_tx_queue_ban_count: u16 = 1u16, or |c: &Config| otry!(c.mining).tx_queue_ban_count.clone(),
			"--tx-queue-ban-count=[C] 'Number of times maximal time for execution (--tx-time-limit) can be exceeded before banning sender/recipient/code.'",

			ARG arg_tx_queue_ban_time: u16 = 180u16, or |c: &Config| otry!(c.mining).tx_queue_ban_time.clone(),
			"--tx-queue-ban-time=[SEC] 'Banning time (in seconds) for offenders of specified execution time limit. Also number of offending actions have to reach the threshold within that time. (default: {arg_tx_queue_ban_time} seconds)'",

			ARG arg_stratum_interface: String = "local", or |c: &Config| otry!(c.stratum).interface.clone(),
			"--stratum-interface=[IP] 'Interface address for Stratum server.'",

			ARG arg_stratum_port: u16 = 8008u16, or |c: &Config| otry!(c.stratum).port.clone(),
			"--stratum-port=[PORT] 'Port for Stratum server to listen on.'",

			ARG_OPTION arg_min_gas_price: u64 = None, or |c: &Config| otry!(c.mining).min_gas_price.clone(),
			"--min-gas-price=[STRING] 'Minimum amount of Wei per GAS to be paid for a transaction to be accepted for mining. Overrides --basic-tx-usd.'",

			ARG_OPTION arg_author: String = None, or |c: &Config| otry!(c.mining).author.clone(),
			"--author=[ADDRESS] 'Specify the block author (aka \"coinbase\") address for sending block rewards from sealed blocks. NOTE: MINING WILL NOT WORK WITHOUT THIS OPTION.'", // Sealing/Mining Option

			ARG_OPTION arg_engine_signer: String = None, or |c: &Config| otry!(c.mining).engine_signer.clone(),
			"--engine-signer=[ADDRESS] 'Specify the address which should be used to sign consensus messages and issue blocks. Relevant only to non-PoW chains.'",

			ARG_OPTION arg_tx_gas_limit: String = None, or |c: &Config| otry!(c.mining).tx_gas_limit.clone(),
			"--tx-gas-limit=[GAS] 'Apply a limit of GAS as the maximum amount of gas a single transaction may have for it to be mined.'",

			ARG_OPTION arg_tx_time_limit: u64 = None, or |c: &Config| otry!(c.mining).tx_time_limit.clone(),
			"--tx-time-limit=[MS] 'Maximal time for processing single transaction. If enabled senders/recipients/code of transactions offending the limit will be banned from being included in transaction queue for 180 seconds.'",
			...
			"--notify-work=[URLS] 'URLs to which work package notifications are pushed. URLS should be a comma-delimited list of HTTP URLs.'",
		...
		["Footprint options"]
			...
			"--fast-and-loose 'Disables DB WAL, which gives a significant speed up but means an unclean exit is unrecoverable.'",

			FLAG flag_scale_verifiers: bool = false, or |c: &Config| otry!(c.footprint).scale_verifiers.clone(),
			"--scale-verifiers 'Automatically scale amount of verifier threads based on workload. Not guaranteed to be faster.'",

			ARG arg_tracing: String = "auto", or |c: &Config| otry!(c.footprint).tracing.clone(),
			"--tracing=[BOOL] 'Indicates if full transaction tracing should be enabled. Works only if client had been fully synced with tracing enabled. BOOL may be one of auto, on, off. auto uses last used value of this option (off if it does not exist).'", // footprint option

			ARG arg_pruning: String = "auto", or |c: &Config| otry!(c.footprint).pruning.clone(),
			"--pruning=[METHOD] 'Configure pruning of the state/storage trie. METHOD may be one of auto, archive, fast: archive - keep all state trie data. No pruning. fast - maintain journal overlay. Fast but 50MB used. auto - use the method most recently synced or default to fast if none synced.'",

			ARG arg_pruning_history: u64 = 64u64, or |c: &Config| otry!(c.footprint).pruning_history.clone(),
			"--pruning-history=[NUM] 'Set a minimum number of recent states to keep when pruning is active..'",

			ARG arg_pruning_memory: usize = 32usize, or |c: &Config| otry!(c.footprint).pruning_memory.clone(),
			"--pruning-memory=[MB] 'The ideal amount of memory in megabytes to use to store recent states. As many states as possible will be kept within this limit, and at least --pruning-history states will always be kept.'",

			ARG arg_cache_size_db: u32 = 32u32, or |c: &Config| otry!(c.footprint).cache_size_db.clone(),
			"--cache-size-db=[MB] 'Override database cache size.'",

			ARG arg_cache_size_blocks: u32 = 8u32, or |c: &Config| otry!(c.footprint).cache_size_blocks.clone(),
			"--cache-size-blocks=[MB] 'Specify the prefered size of the blockchain cache in megabytes.'",

			ARG arg_cache_size_queue: u32 = 40u32, or |c: &Config| otry!(c.footprint).cache_size_queue.clone(),
			"--cache-size-queue=[MB] 'Specify the maximum size of memory to use for block queue.'",

			ARG arg_cache_size_state: u32 = 25u32, or |c: &Config| otry!(c.footprint).cache_size_state.clone(),
			"--cache-size-state=[MB] 'Specify the maximum size of memory to use for the state cache.'",

			ARG arg_db_compaction: String = "auto", or |c: &Config| otry!(c.footprint).db_compaction.clone(),
			"--db-compaction=[TYPE] 'Database compaction type. TYPE may be one of: ssd - suitable for SSDs and fast HDDs; hdd - suitable for slow HDDs; auto - determine automatically.'",

			ARG arg_fat_db: String = "auto", or |c: &Config| otry!(c.footprint).fat_db.clone(),
			"--fat-db=[BOOL] 'Build appropriate information to allow enumeration of all accounts and storage keys. Doubles the size of the state database. BOOL may be one of on, off or auto.'",

			ARG_OPTION arg_cache_size: u32 = None, or |c: &Config| otry!(c.footprint).cache_size.clone(),
			"--cache-size=[MB] 'Set total amount of discretionary memory to use for the entire system, overrides other cache and queue options.'",

			ARG_OPTION arg_num_verifiers: usize = None, or |c: &Config| otry!(c.footprint).num_verifiers.clone(),
			"--num-verifiers=[INT] 'Amount of verifier threads to use or to begin with, if verifier auto-scaling is enabled.'",

		["Import/export options"]
			FLAG flag_no_seal_check: bool = false, or |_| None,
			"--no-seal-check 'Skip block seal check.'",
			...
			"--from=[BLOCK] 'Export from block BLOCK, which may be an index or hash.'", // Import/Export Options:
			...
			"--format=[FORMAT] 'For import/export in given format. FORMAT must be one of 'hex' and 'binary'. (default: {arg_format:?} = Import: auto, Export: binary)'",
		...
		["Snapshot options"]
			...
			"--no-periodic-snapshot 'Disable automated snapshots which usually occur once every 10000 blocks.'",

			ARG arg_at: String = "latest", or |_| None,
			"--at=[BLOCK] 'Take a snapshot at the given block, which may be an index, hash, or 'latest'. Note that taking snapshots at non-recent blocks will only work with --pruning archive'", // Snapshot Option
		...
		["Whisper options"]
			...
			"--whisper 'Enable the Whisper network.'",
		...
		["Legacy options"]
			...
			"--dapps-apis-all 'Dapps server is merged with RPC server. Use --jsonrpc-apis.'",

			ARG_OPTION arg_dapps_port: u16 = None, or |c: &Config| otry!(c.dapps).port.clone(),
			"--dapps-port=[PORT] 'Dapps server is merged with RPC server. Use --jsonrpc-port.'",

			ARG_OPTION arg_dapps_interface: String = None, or |c: &Config| otry!(c.dapps).interface.clone(),
			"--dapps-interface=[IP] 'Dapps server is merged with RPC server. Use --jsonrpc-interface.'",

			ARG_OPTION arg_dapps_hosts: String = None, or |c: &Config| otry!(c.dapps).hosts.as_ref().map(|vec| vec.join(",")),
			"--dapps-hosts=[HOSTS] 'Dapps server is merged with RPC server. Use --jsonrpc-hosts.'",

			ARG_OPTION arg_dapps_cors: String = None, or |c: &Config| otry!(c.dapps).cors.clone(),
			"--dapps-cors=[URL] 'Dapps server is merged with RPC server. Use --jsonrpc-cors.'",
